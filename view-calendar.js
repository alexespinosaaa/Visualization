...
export function init(container, data, state, dispatch) {
    ...
    // Setup Gradients for Heatmap mode
    const defs = svg.append("defs");
    const redGrad = defs.append("linearGradient").attr("id", "grad-red");
    redGrad.append("stop").attr("offset", "0%").attr("stop-color", "#fff5f0");
    redGrad.append("stop").attr("offset", "100%").attr("stop-color", "#a50f15");
    ...
    svg.selectAll(".month-label").data(monthNames).join("text")
        ...
        .attr("y", -10).text(d => d).style("font-size", "11px").style("fill", "#666").style("font-weight", "bold");

    const xAxis = d3.axisBottom(x).tickValues([1, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 52]).tickFormat(d => `W${d}`);
...
export function update(container, data, state) {
    ...
    // Determine Logic: Heatmap OR Event Highlight
    let isEventMode = state.selectedEventType !== null;
    let eventColor = "#ccc";
    if (state.selectedEventType === 'flu') eventColor = "#9b59b6"; // Purple
    if (state.selectedEventType === 'strike') eventColor = "#e67e22"; // Orange
    if (state.selectedEventType === 'donation') eventColor = "#3498db"; // Blue
    ...
    svg.selectAll(".cell")
        ...
        .on("mouseover", (event, d) => {
            ...
            tooltip.style("opacity", 1).html(`
                ...
                ${d.eventType !== 'none' ? `<span style="color:red">${d.eventType.toUpperCase()}</span><br>` : ''}
                ${state.metric}: ${d[state.metric].toFixed(1)}
        ...
        .attr("fill", d => {
            // *** MAJOR CHANGE: Color Logic ***
            if (isEventMode) {
                // If this week matches the event, color it. Else grey.
                return d.eventType === state.selectedEventType ? eventColor : "#f1f2f6";
            }
            // Standard Heatmap
            return heatScales[state.metric](d[state.metric]);
        })
        .attr("opacity", d => {
